...
val ENROLLMENTACTIVITY = 104
val INCIDENTLOGACTIVITY = 105
val PARENT_HOME = 106
val ADD_CHILD = 107
...
object WebServices {
    ...
    const val GET_ALL_INCIDENTS = "Parent/GetAllIncidentsByChildID"

    const val GET_ENROLLMENT_DATA = "Parent/GetStudentClassEnrollment"

    const val GET_ALL_STUDENT_OF_PARENT = "Parent/GetAllStudentsOfParent"

    const val GET_CLASS_ATTENDENCE = "Parent/GetAttendanceListforparent"
    const val STUDENT_BREAK_LIST = "Teacher/GetStudentBreakLogs"
    const val GET_CLASS_DAILY_SHEET = "Parent/GetDailySheetForParent"
    ...
    const val SAVE_STUDENT = "Parent/SaveStudent"
    const val SAVE_STUDENT_GUARDIAN = "Parent/SaveStudentGaurdians"
    const val GET_RELATION_TYPE = "Masters/GetRelationType"
    const val GET_IMMUNIZATION_TYPE = "Masters/GetImmunizationType"
    const val SAVE_STUDENT_IMMUNIZATION = "Parent/SaveStudentImmunization"
    const val GET_ALLERGY_TYPE = "Masters/GetAllergyType"
    const val GET_ALLERGY_NAME_TYPE = "Masters/GetAllergyName"
    const val GET_REACION_TYPE = "Masters/GetAllergyReactionType"
    const val SAVE_STUDENT_ALLERGIES = "Parent/SaveStudentAllergies"
    const val SAVE_STUDENT_MEDICATION = "Parent/SaveStudentMedication"
    const val SAVE_STUDENT_DISABILITY = "Parent/SaveStudentDisabilities"
    const val GET_DIAPERDATA = "Classes/GetParticularStudentActivityDiaperChanges"
    const val GET_POSTS_BY_CHILD_ID = "Parent/GetAllPostActivitiesByChildID"
    const val SAVE_PARENT_DASHBOARD_IMAGE_LIKED_INFO = "Parent/SaveParentDashboardImagedLikeInformation"
    const val SAVE_PARENT_DASHBOARD_VIDEO_LIKED_INFO = "Parent/SaveParentDashboardVideoLikeInformation"
    const val GET_STUDENT_INFO = "Agency/GetStudentInformation"
    const val GET_ALL_DOSAGE_REPEAT = "Masters/GetAllDoseRepeat"
    const val SAVE_STUDENT_ENROLLMENT = "Parent/SaveStudentEnrollment"
    const val GET_PARENT_INFORMATION = "Parent/GetParentInformation"
    const val SAVE_PARENT_INFO = "Parent/SaveParentInformation"
    const val GET_PARENT_ACCORDING_TO_LOGIN = "Parent/GetParentAccordingtoLogin"
    const val GET_STRIPE_DETAILS_FOR_AGENCY = "Masters/GetStripeDetailsForAgency"
    const val PAYMENT_DETAILS = "Masters/PaymentDetails"
    const val GET_PAYMENT_DETAILS = "Masters/GetPaymentDetailsForParent"
    const val GET_DUE_PAYMENT_LIST = "Masters/GetDuePaymentAccordingToParent"
    ...
    const val UPDATE_INCIDENT = "Agency/UpdateIncidentDetailsByParent"
