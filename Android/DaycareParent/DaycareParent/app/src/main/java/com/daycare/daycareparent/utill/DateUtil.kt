...
fun datepicker(context: Context, et: TextInputEditText) {
    ...
    val mYear = getYearInt(et.text?.toString())// c.get(Calendar.YEAR)
    val mMonth = getMonthInt(et.text?.toString())//c.get(Calendar.MONTH)
    val mDay = getDayInt(et.text?.toString())//c.get(Calendar.DAY_OF_MONTH)
...
fun datepickerCustom(
    ...
    if (state == START_DATE) {
        ...
        dp.minDate = convertTimeInMillis(minDate)
    }
...
fun getMonthInt(todaysDate: String?): Int {
    return if (!todaysDate.isNullOrEmpty()) {
        convertDate(todaysDate, incidentDisplayDate, monthInt).toInt() - 1
    } else {
...
fun getDayInt(todaysDate: String?): Int {
    return if (!todaysDate.isNullOrEmpty()) {
        convertDate(todaysDate, incidentDisplayDate, numDate).toInt()
    } else {
...
fun getYearInt(todaysDate: String?): Int {
    return if (!todaysDate.isNullOrEmpty()) {
        convertDate(todaysDate, incidentDisplayDate, yearInt).toInt()
    } else {
...
fun convertUTCDate(date: String, currentFormat: SimpleDateFormat, convertFormat: SimpleDateFormat): String {
    try {
         currentFormat.timeZone = TimeZone.getTimeZone("UTC")
        val d = currentFormat.parse(date)
        return convertFormat.format(d)
...
fun convertTimeInMillis(givenDateString: String?): Long {
    ...
    val dateFormat = incidentDisplayDate
//    try {
    return if (givenDateString != null) {
