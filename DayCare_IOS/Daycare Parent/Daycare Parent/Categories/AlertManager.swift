...
open class AlertButton: Any {
    ...
    init(style: UIAlertAction.Style = .default, title: String = Macros.alertMessages.okString, buttonClicked: BlankBlock? = nil) {
        self.style = style
        self.title = title
...
open class AlertManager: Any {
    ...
    open class func showOKAlert(_ type: UIAlertAction.Style = .default, withTitle title: String?, withMessage message: String?, onViewController vc: UIViewController, animatedly: Bool = true, presentationCompletionHandler: BlankBlock? = nil, returnBlock: ButtonClickedResponse? = nil) -> UIAlertController {
        return showAlertWithSimilarButtonsType(type, withTitle: title, withMessage: message, buttonsTitles: [Macros.alertMessages.okString], onViewController: vc, animatedly: animatedly, presentationCompletionHandler: presentationCompletionHandler, returnBlock: returnBlock)
    }
        ...
        DispatchQueue.main.async {
            ...
            if (UIDevice.current.userInterfaceIdiom == .pad){
                if alert.responds(to: #selector(getter: vc.popoverPresentationController)) {
                    ...
                    alert.popoverPresentationController?.sourceRect = CGRect(x: 0, y: UIScreen.main.bounds.size.height, width: UIScreen.main.bounds.size.width, height: 320)
    ...
    static func isValidPassword(value: String) -> Bool {
        let passwordRegex = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)[a-zA-Z\\d\\W]{8,}$"
