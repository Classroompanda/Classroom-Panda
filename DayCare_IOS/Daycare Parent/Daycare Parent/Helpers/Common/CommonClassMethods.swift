...
class CommonClassMethods: NSObject {
    ...
    //Time From Time String
    class func timeFromTimeString(time: String) -> String {
        let dateFormatter        = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss"
    ...
    class func dateObjectWithoutTimeFromDateStringS(date: String) -> Date {
        let dateFormatter        = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
        let newDate           = dateFormatter.date(from: date)
        return self.convertDateWithoutTime(date: newDate ?? Date())
...
//        dateFormatter.timeZone   = TimeZone(abbreviation: "UTC")
    ...
    //used in creating recurrence
    
  class func toUTCString(withCurrentFormat currentFormat: String? = nil, date:Date, outputFormat: String = DateFormat.utcDateFormat) -> String? {
        let dateFormatter = DateFormatter()
    ...
    //date from strig to server readable Format
    class func convertDateToServerReadableFormatFromString(date:String) -> String {
        let dateFormatter        = DateFormatter()
        dateFormatter.dateFormat = "yyyy-MM-dd'T'HH:mm:ss"
        let newDate           = dateFormatter.date(from: date)
        return self.convertDateToServerReadableFormat(date: newDate ?? Date())
    }
    ...
    //date from dateString
    class func dateFromDateStringS(date:String) -> String{
        let dateFormatter        = DateFormatter()
        dateFormatter.dateFormat = (date.length() == 19) ? "yyyy-MM-dd'T'HH:mm:ss" : (date.length() == 23) ? "yyyy-MM-dd'T'HH:mm:ss.SSS" : "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
        let newDate           = dateFormatter.date(from: date)
        return self.dateFromDateFormat(date: newDate ?? Date())
    ...
    //date from dateString
    class func dateFromDateStringSSS(date:String) -> String{
        let dateFormatter        = DateFormatter()
        dateFormatter.dateFormat = (date.length() == 19) ? "yyyy-MM-dd'T'HH:mm:ss" : (date.length() == 23) ? "yyyy-MM-dd'T'HH:mm:ss.SSS" : "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'"
        let newDate           = dateFormatter.date(from: date)
        return self.dateFromDateFormat(date: newDate ?? Date())
...
//        var maximumDate = CommonClassMethods.convertDateToServerReadableFormat(date: dateObject)
//        let dateComponents = DateComponents(year: Int(CommonClassMethods.yearFromDateString(date: maximumDate)), month: Int(CommonClassMethods.monthFromDateString(date: maximumDate)))
//        let calendar = Calendar.current
//        let date = calendar.date(from: dateComponents)!
//        let range = calendar.range(of: .day, in: .month, for: date)!
//        let numDays = String(range.count)
//        maximumDate = CommonClassMethods.replace(myString: maximumDate, 8, numDays.character(at: 0) ?? "0")
//        maximumDate = CommonClassMethods.replace(myString: maximumDate, 9, numDays.character(at: 1) ?? "0")
//        return maximumDate
//    }
//
//    //get MinimumDate Of month
//    class func getMinimumDateOfMonth(dateObject:Date) -> String {
//        var minimumDate = CommonClassMethods.convertDateToServerReadableFormat(date: dateObject)
...
//    }
    ...
    class func getMinimumDateOfMonth(date: Date) -> String {
        let calendar = Calendar.current
        let components = calendar.dateComponents([.year, .month], from: date)
        return CommonClassMethods.convertDateToServerReadableFormat(date:calendar.date(from: components) ?? Date())
    }
    
    class func getMaximumDateOfMonth(date: Date) -> String {
        let calendar = Calendar.current
        var component = DateComponents()
        component.day = -1
        component.month = 1
        return CommonClassMethods.convertDateToServerReadableFormat(date: calendar.date(byAdding: component, to: date) ?? Date())
    }
