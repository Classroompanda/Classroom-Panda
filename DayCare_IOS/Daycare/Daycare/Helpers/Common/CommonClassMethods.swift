...
class CommonClassMethods: NSObject {
  ...
  class func convertDateToServerReadableFormatGET(date:Date, toFormat: String = "yyyy-MM-dd'T'HH:mm:ss.SSS'Z'") -> String{
    ...
    // shiwani, uncommented UTC , this is used many times in the code
    dateFormatter.timeZone   = TimeZone(abbreviation: "UTC")
    let dateString           = dateFormatter.string(from: date)
    return dateString
  }
...
class func setCurrentTimeWithDate(_ date: Date) -> Date {
  let gregorian = NSCalendar(calendarIdentifier: NSCalendar.Identifier.gregorian)!
  var componentsDate = gregorian.components([.year, .month, .day, .hour, .minute, .second], from: date as Date)
  let components = gregorian.components([.year, .month, .day, .hour, .minute, .second], from: Date())

  // Change the time to 9:30:00 in your locale
  componentsDate.hour = components.hour
  componentsDate.minute = components.minute
  componentsDate.second = components.second

  let dateObj = gregorian.date(from: componentsDate)!
  return dateObj
}
  // Shiwani
  class func dateToStringDay(date:Date) -> String{
    let dateFormatter        = DateFormatter()
