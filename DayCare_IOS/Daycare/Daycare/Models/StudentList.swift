...
class StudentList: NSObject {
    ...
    public class func modelsFromDictionaryArray(array:Array<Dictionary<String,Any>>) -> [StudentList]
    {
        var models:[StudentList] = []
        for item in array
        {
            models.append(StudentList(dictionary: item)!)
        }
        return models
    }
