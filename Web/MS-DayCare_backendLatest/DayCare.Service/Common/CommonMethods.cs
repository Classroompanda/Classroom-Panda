...
namespace DayCare.Service.Common
{
    public class CommonMethods
    {
        ...
        public TokenModel GetTokenDataModel(HttpContext request)
        {
            TokenModel token = new TokenModel();

            token = new TokenModel() { UserID = 5, OrganizationID = 1, Timezone = "India Standard Time", IPAddress = "203.129.220.76", LocationID = 1, RoleID = 1, DomainName = HCOrganizationConnectionStringEnum.Host, Request = request };
        ...
        public DateTime ConvertUtcTime(DateTime Date, string Timezone)
        {
            ...
            {
                TimeZoneInfo timeInfo = TimeZoneInfo.FindSystemTimeZoneById("India Standard Time"); 
                DateTime userTime = TimeZoneInfo.ConvertTimeToUtc(Date, timeInfo);
                return userTime;
            }
        }
        ...
        public string GetExtenstion(string mimeType)
        {
            if (mimeType == null)
            {
        ...
        public int Decryption(string userid)
        {
            userid = userid.Replace("[", "/");
            byte[] inputArray = Convert.FromBase64String(userid);
            TripleDESCryptoServiceProvider tripleDES = new TripleDESCryptoServiceProvider();
            tripleDES.Key = UTF8Encoding.UTF8.GetBytes("sblw-3hn8-sqoy19"); //Key for Decryption
            tripleDES.Mode = CipherMode.ECB;
            tripleDES.Padding = PaddingMode.PKCS7;
            ICryptoTransform cTransform = tripleDES.CreateDecryptor();
            byte[] resultArray1 = cTransform.TransformFinalBlock(inputArray, 0, inputArray.Length);
            tripleDES.Clear();
            var decodedUserId = UTF8Encoding.UTF8.GetString(resultArray1); //Dycrypted UserID in string form
            return Convert.ToInt32(decodedUserId); //Dycrypted UserID in integer form
        }

        public string Encryption(long userid)
        {
            //Encrypt the UserID 
            string StringUserId = Convert.ToString(userid);
            byte[] inputArray = UTF8Encoding.UTF8.GetBytes(StringUserId);
            TripleDESCryptoServiceProvider tripleDES = new TripleDESCryptoServiceProvider();
            tripleDES.Key = UTF8Encoding.UTF8.GetBytes("sblw-3hn8-sqoy19");//Key for Encryption
            tripleDES.Mode = CipherMode.ECB;
            tripleDES.Padding = PaddingMode.PKCS7;
            ICryptoTransform cTransform = tripleDES.CreateEncryptor();
            byte[] resultArray = cTransform.TransformFinalBlock(inputArray, 0, inputArray.Length);
            tripleDES.Clear();
            var encodedUserId = Convert.ToBase64String(resultArray, 0, resultArray.Length);//Encrypted UserID in string form     
            encodedUserId = encodedUserId.Replace("/", "[");
            return encodedUserId.ToString().Replace("=", "%3d");//Replace '=' sign with %3d
        }
