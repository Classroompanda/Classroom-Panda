...
import { EventVM } from '../../../teacher/shared/view-model/eventVM';
...
declare var $: any;
@Component({
  selector: 'app-parent-event-planner',
  templateUrl: './parent-event-planner.component.html',
  styleUrls: ['./parent-event-planner.component.css']
})
export class ParentEventPlannerComponent implements OnInit {
  display: boolean;
  cars: any[] = [];
  events: any[];
  options: any;
  ...
  createEventForm() {
    if (this.eventId === 0) {
      this.calenderForm = this.fb.group({
        ...
        repeat: [{ value: this.repeatNeverID, disabled: true }],
        endson: [{ value: '', disabled: true }]
      });
    }
  }
  ...
  editEventForm() {
    if (this.eventId !== 0) {
      this.calenderForm = this.fb.group({
        startdate: [{ value: this.eventVM.start, disabled: true }],
        enddate: [{ value: this.eventVM.end, disabled: true }, Validators.required],
        starttime: [{ value: this.eventVM.startTime, disabled: true }, Validators.required],
        endtime: [{ value: this.eventVM.endTime, disabled: true }, Validators.required],
        class: [{ value: this.classListArray, disabled: true }, Validators.required],
        title: [{ value: this.eventVM.title, disabled: true }, Validators.required],
        description: [{ value: this.eventVM.description, disabled: true }, Validators.required],
        repeat: [{ value: this.eventVM.plannerRepeatTypeID, disabled: true }],
        endson: [{ value: this.eventVM.endsOn, disabled: true }]
      });
    }
  }
