...
declare var $: any;
@Component({
  selector: 'app-parent-meal-planner',
  templateUrl: './parent-meal-planner.component.html',
  styleUrls: ['./parent-meal-planner.component.css']
})
export class ParentMealPlannerComponent implements OnInit {
  cars: any[] = [];
  display = false;
  mealVM: MealVM = {};
  events: any[];
      ...
      if (res.body.statusCode === 200) {
        if (res.body.data[0].involvedClass.length !== 0) {
          res.body.data[0].involvedClass.forEach(element => {
            ...
            console.log('this.classListArray1', this.showClassName);
          });
        }
